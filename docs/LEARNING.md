...
| H6 | Inhibition ≠ grouping ≠ silence; silence drops notify, eval continues | PR6 | ✅ silenced alert stays `firing` in the group and is listed by `GET /api/v2/alerts?silenced=true`; webhook goes silent and resumes on expiry |
...
- **PR6:** silences — `am.Silence` = matchers (`=`, `!=`, `=~`, `!~`; missing label matches `!=`/`!~`) + time window (`pending`/`active`/`expired`). The dispatcher's `flush()` gates on `Silences.Mutes`: muted alerts are filtered out of the sendable batch, **but the group, nflog, and engine state are untouched** — so a silenced alert keeps evaluating and stays `firing`, and when the silence expires the unchanged set sends at the next flush (H6). HTTP: `POST/GET /api/v2/silences`, `GET/DELETE /api/v2/silences/{id}` (delete = expire), `GET /api/v2/alerts?silenced=true|false` proves silence ≠ deletion. Contrast test: pausing a rule stops eval entirely (no instance, no group) — that is *not* a silence.
